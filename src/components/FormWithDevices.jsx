...
const FormWithDevices = () => {
  ...
  const [email, setEmail] = useState(""); // Dodane
  const [phoneNumber, setPhoneNumber] = useState(""); // Dodane
  const [message, setMessage] = useState(""); // Dodane
  const [error, setError] = useState(""); // Dodane
  ...
  useEffect(() => {
    if (chosenDevices.length > 0) {
      const devices = chosenDevices
        .map((device) => {
          return `Urządzenie: ${device.name}, Moc: ${device.power}W, Ilość: ${device.quantity}`;
        })
        .join("\n");

      console.log(devices);
      setMessage(devices + `\nŁączna moc: ${sumPower}W`);
    }
  }, []);
    ...
    if (emailRegex.test(email) && phoneRegex.test(phoneNumber)) {
      emailjs
        .sendForm(
          ...
          process.env.REACT_APP_EMAILJS_DEVICESTEMPLATE_ID,
          formRef.current,
          {
  ...
  return (
    <div className="w-full">
      <button onClick={() => console.log(devicesString)}>click</button>
      <form
        onSubmit={sendEmail}
        ref={formRef}
        className="flex flex-col md:w-[38%] w-[90%] mx-auto"
      >
        <input type="hidden" value={devicesString} />
        <input
          type="text"
          name="email"
          value={email}
        ...
        <textarea
          ...
          className="bg-gray-800 text-gray-100 text-[20px] px-4 py-3 rounded-2xl shadow-sm shadow-gray-700 mt-4 focus:outline-none"
        ></textarea>
        {error && (
...
export default FormWithDevices;
